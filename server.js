...
const {generateLocationMessage,formatMessage}  = require('./utils/messages');
const {
  userJoin,
  getCurrentUser,
  userLeave,
...
io.on('connection', socket => {
  socket.on('joinRoom', ({ username, room }) => {
    ...
    //New user added
    socket.emit('message', formatMessage(user.room, 'Welcome to ChatRoom!'));
    ...
    socket.broadcast
      ...
      .emit(
        'message',
        formatMessage(user.room, `${user.username} has joined the chat`)
      );
